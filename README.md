...
Five minutes, start to a working screen. Every step here is safe — nothing in this section can place
a trade or spend money.

**1. Start Docker Desktop.** Look for the whale icon in your menu bar. Wait until it stops animating.

**2. Open Terminal and run:**

```bash
cd /Users/hemantpatel/projects/IB
docker compose up
```

**Leave this window open.** Closing it stops the system. The first run takes a few minutes; later
ones are quick. Wait until the scrolling text settles down.

**3. Open the dashboard:** **http://localhost:3001**

**4. Confirm these four things before trusting anything on screen:**
...
| **Broker**         | Green text      | Red means no live prices — see §8                           |
| **Alerts (top)**   | Nothing there   | Red or amber bars — see [§8](#8-when-something-looks-wrong) |

Confusingly, kill switch **ARMED means inactive** — it is not blocking anything. That's the normal
resting state.

**5. To stop:** press `Ctrl-C` in the Terminal window. Stopping is always safe. Nothing is sold, and
...
Press `Ctrl-C` in that Terminal window. Or from another window:
...
All settings live in a file called `.env`. **`EXECUTION_MODE=SHADOW` is the line that keeps this
system from trading.** Do not change it. The two others worth recognising:

- `DATABASE_URL` — present means the database is in use (`DURABLE`). Absent means memory only.
- `IB_HOST` — present means connect to real IB. Absent means run on test data.
...
| `PRD.md`           | Engineer          | The full specification                    |
| `CLAUDE.md`        | Engineer          | Architecture and invariants               |
| `stories.md`       | Engineer          | The build plan                            |
